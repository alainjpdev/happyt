...
async function main() {
  ...
  const user = await prisma.user.findUnique({ where: { email: 'student@algorithmics.com' } });
  const coordinator = await prisma.user.findUnique({ where: { email: 'teacher@algorithmics.com' } });
  const admin = await prisma.user.findUnique({ where: { email: 'admin@algorithmics.com' } });

  if (!user || !coordinator || !admin) {
