...
import { BookOpen, Calendar, ClipboardList, User, LogOut, Home, ChevronLeft, ChevronRight } from 'lucide-react';
        ...
        <div className="flex flex-col h-full">
          ...
          <div className={`${collapsed ? 'px-2 py-4' : 'px-6 py-4'} border-b border-border`}>
            <div className={`flex items-center ${collapsed ? 'justify-center' : ''}`}>
              <div className={`${collapsed ? 'w-12 h-12' : 'w-10 h-10'} ${collapsed ? 'bg-gray-200' : 'bg-primary/20'} rounded-full flex items-center justify-center flex-shrink-0`}>
                <User className={`${collapsed ? 'w-8 h-8' : 'w-6 h-6'} ${collapsed ? 'text-gray-800' : 'text-primary'}`} />
              </div>
              {!collapsed && (
                <div className="ml-3">
